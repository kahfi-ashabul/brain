...
export function useTheme() {
  const [theme, setTheme] = useState<Theme>(() => {
    ...
    const stored = localStorage.getItem('kei-theme') as Theme | null;
  ...
  useEffect(() => {
    ...
    localStorage.setItem('kei-theme', theme);
  }, [theme]);
...
export function useHighScores() {
  const getScore = useCallback((game: string, key: string = 'default') => {
    const data = localStorage.getItem(`kei-hs-${game}-${key}`);
  ...
  const saveScore = useCallback((game: string, value: number, key: string = 'default') => {
    ...
    if (value > current) {
      localStorage.setItem(`kei-hs-${game}-${key}`, String(value));
      return true;
    }
    return false;
  }, [getScore]);
...
export function useStats() {
  const getStats = useCallback(() => {
    const data = localStorage.getItem('kei-stats');
  ...
  const saveStats = useCallback((updates: Record<string, number>) => {
    ...
    localStorage.setItem('kei-stats', JSON.stringify(next));
    return next;
  }, [getStats]);
